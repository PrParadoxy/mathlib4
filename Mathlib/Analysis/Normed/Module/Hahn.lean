...
theorem exists_dual_vector'''' (x : E) : ∃ g : StrongDual 𝕜 E, ‖g‖ ≤ 1 ∧ g x = ‖x‖ := by
  ...
  · let coord := (ofHomothety (LinearEquiv.toSpanNonzeroSingleton 𝕜 E x (by aesop)) ‖x‖ hx
    ...
    let f := (‖x‖ : 𝕜) • coord
    have hf : ‖f‖ = 1 := by
      simp only [algebraMap_smul, norm_smul, norm_norm, f]
      convert mul_inv_cancel₀ ((ne_of_lt hx).symm)
      ...
      · exact ContinuousLinearMap.opNorm_le_bound _ (by simp) (fun q => (h_inv q).le)
      · let z : 𝕜 ∙ x := ⟨(1 : 𝕜) • x, by simp⟩
        apply (mul_le_mul_iff_left₀ (by simp [z, hx] : 0 < ‖(z : E)‖)).mp
        rw [← h_inv]
        apply ContinuousLinearMap.le_opNorm
    obtain ⟨g, hg⟩ := exists_extension_norm_eq (𝕜 ∙ x) f
    refine ⟨g, by rw [hg.2, hf], ?_⟩
    calc
      g x = g (⟨x, mem_span_singleton_self x⟩ : 𝕜 ∙ x) := by rw [Submodule.coe_mk]
      ...
      _ = ‖x‖ := by
        ...
        conv_lhs => arg 2; apply LinearEquiv.coord_self 𝕜 E x (by aesop)
        simp
  · replace hx : ‖x‖ = 0 := le_antisymm (by simp_all) (norm_nonneg x)
