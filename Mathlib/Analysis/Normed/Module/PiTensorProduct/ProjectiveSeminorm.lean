...
section containInstances

-- TBD: These need to be made `def`s, or "non-instances"
noncomputable local instance projectiveSeminormedAddCommGroup :
  SeminormedAddCommGroup (⨂[𝕜] i, E i) :=
  AddGroupSeminorm.toSeminormedAddCommGroup projectiveSeminorm.toAddGroupSeminorm

noncomputable local instance projectiveNormedSpace :
