...
variable {ι R : Type*} {M : ι → Type*} {κ : ι → Type*} [Finite ι] [DecidableEq ι] [CommSemiring R]
   [DecidableEq R] [∀ i, AddCommMonoid (M i)] [∀ i, Module R (M i)] [(i : ι) → DecidableEq (κ i)]
...
@[simp]
theorem Basis.piTensorProduct_repr_tprod_apply (b : Π i, Basis (κ i) R (M i))
    (x : Π i, M i) (p : Π i, κ i) :
    haveI := Fintype.ofFinite ι
