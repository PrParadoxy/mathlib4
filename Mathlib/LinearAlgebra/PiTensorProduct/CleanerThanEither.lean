...
def tmulFinSucc :
  (⨂[R] (i : Fin n.succ), s i) ≃ₗ[R] ((⨂[R] (i : Fin n), s i.castSucc) ⊗[R] (s (last n))) :=
  (reindex R s finSumFinEquiv.symm) ≪≫ₗ
  (tmulEquivDep R (fun i ↦ s (finSumFinEquiv i))).symm ≪≫ₗ
  (TensorProduct.congr (LinearEquiv.refl _ _) (subsingletonEquivDep 0))

@[simp]
theorem tmulFinSucc_tprod (f : (i : Fin n.succ) → s i) :
    tmulFinSucc (⨂ₜ[R] i, f i) = (⨂ₜ[R] i, f (castSucc i)) ⊗ₜ[R] f (last n) := by
    simp only [tmulFinSucc]
    simp only [Nat.succ_eq_add_one, finSumFinEquiv_apply_left, finSumFinEquiv_apply_right,
      Equiv.symm_symm, isValue, LinearEquiv.trans_apply]
    sorry
...
def tprodFinTprodEquiv :
  ...
  | succ m ih => exact tmulFinSucc ≪≫ₗ TensorProduct.congr ih (LinearEquiv.refl _ _) ≪≫ₗ
...
theorem tprodFinTprodEquiv_tprod (f : (k : Fin n) → (i : Tf k) → s k i) :
  ...
  | succ m ih =>
    ...
    simp only [tprodFinTprodEquiv, LinearEquiv.trans_apply, tmulFinSucc_tprod,
