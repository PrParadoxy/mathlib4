...
theorem eval_dualpairing_surjective :
  ...
  have hker: ⨅ i, LinearMap.ker (vf i : AlgWeakDual R V →ₗ[R] R)
    ≤ LinearMap.ker (f : AlgWeakDual R V →ₗ[R] R) := by
    intro dv hdv
    ...
    by_contra! hc
    replace hdv : ∀ v ∈ sv, dv v = 0 := fun v hv => by
...
lemma exists_separating_vector {dv : AlgWeakDual ℝ V}
  ...
  exact ⟨(StrongAlgWeakDualEquiv _ _).symm v, by simp [hvz],
